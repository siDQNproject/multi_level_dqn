...
function setup(_opt,level,skillplay_num_of_skills)
    assert(_opt)
    level = level or 1
    --preprocess options:
    --- convert options strings to tables
    ...
    _opt.name = _opt.name .. "_" .. level -- SI changed name of network
    ...
    -- Original : game_actions:  , game_actions[1]=0 ,  , game_actions[2]=1 ,  , game_actions[3]=2 ,  , game_actions[4]=3 ,  , game_actions[5]=4 ,  , game_actions[6]=5 ,
    -- Where :
    --   game_actions[1]=0 = Do nothing
    --   game_actions[2]=1 = Do nothing
    --   game_actions[3]=2 = Up-right
    --   game_actions[4]=3 = Down-right
    --   game_actions[5]=4 = Up-left
    --   game_actions[6]=5 = Down-left

    original_actions_last_ind = 0
    for k in pairs(gameActions) do
        original_actions_last_ind = original_actions_last_ind + 1
    end
    for ind_offset=1,skillplay_num_of_skills do
        new_action_ind = original_actions_last_ind+ind_offset
        gameActions[new_action_ind] = (new_action_ind - 1)
    end
    ...
    return gameEnv, gameActions, agent, opt, original_actions_last_ind
end
